...
function test_single_level(
    ...
    @testset "bilinear interpolation on single level with $FT, nsource = ($nsourcex, $nsourcey), ntarget = ($ntargetx, $ntargety)" begin
        ...
        if DA <: MtlArray
            trial_interpolator =
                @benchmark Metal.@sync interpolatebilinear!($ftarget, $bilinear, $fsource)
        else
            trial_interpolator =
...
xrange, yrange, nsource, ntarget, toler = get_dims_singlelevel(Float32)
trial_sl_mtl_ft32_cons, trial_sl_mtl_ft32_interp =
    test_single_level(MtlArray, Float32, xrange, yrange, nsource, ntarget, toler = toler)
println(
    "interpolation time = $(Statistics.median(trial_sl_mtl_ft32_interp)) on Apple GPU for single level with Float32\nConstructor time = $(Statistics.median(trial_sl_mtl_ft32_cons))",
)
@show "--------------------------------------------"
xrange, yrange, zrange, nsource, ntarget, nlevels, toler = get_dims_multilevel(Float32)
trial_ml_mtl_ft32_cons, trial_ml_mtl_ft32_interp = test_multilevel(
    MtlArray,
    Float32,
    xrange,
    yrange,
...
println(
    "interpolation time = $(Statistics.median(trial_ml_mtl_ft32_interp)) on Apple GPU for $nlevels levels with Float32\nConstructor time = $(Statistics.median(trial_ml_mtl_ft32_cons))",
)
@show "--------------------------------------------"
