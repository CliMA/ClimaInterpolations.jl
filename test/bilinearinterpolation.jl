...
function test_multilevel(
    ...
    @testset "bilinear interpolation on $nlevels levels with $FT, nsource = ($nsourcex, $nsourcey), ntarget = ($ntargetx, $ntargety)" begin
        ...
        sourcemesh = (
            x = DA([
                xsourcecpu[j] for i in 1:nlevels, j in 1:nsourcex,
                k in 1:nsourcey
            ]),
            y = DA([
                ysourcecpu[k] for i in 1:nlevels, j in 1:nsourcex,
                k in 1:nsourcey
            ]),
            z = DA([
        ...
        targetmesh = (
            x = DA([
                xtargetcpu[j] for i in 1:nlevels, j in 1:ntargetx,
                k in 1:ntargety
            ]),
            y = DA([
                ytargetcpu[k] for i in 1:nlevels, j in 1:ntargetx,
                k in 1:ntargety
            ]),
            z = DA([
