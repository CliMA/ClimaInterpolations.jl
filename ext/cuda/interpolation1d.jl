...
function interpolate1d!(
    ...
    @assert size(ftarget, 1) == size(xtarget, 1) # check if column length is same between ftarget and xtarget
    @assert size(fsource, 1) == size(xsource, 1) # check if column length is same between fsource and xsource
